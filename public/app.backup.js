...
const nav=[['dashboard','⌂','Dashboard'],['profile','◉','My Profile'],['timetable','▦','Timetable'],['attendance','✓','Attendance'],['subjects','▤','Subjects'],['tasks','☑','Study Plan'],['tests','▣','Tests'],['focuslearn','▶','FocusLearn'],['notifications','🔔','Notifications']];
function renderShell(){app.innerHTML=`<div class="shell"><aside id="sidebar"><div class="brand side-brand"><span class="logo">SC</span><span>StudyConnect</span></div><div class="side-sub">ACADEMIC COMMAND CENTER</div><nav>${nav.map(n=>`<button data-view="${n[0]}"><span>${n[1]}</span>${n[2]}</button>`).join('')}<button id="aiBtn"><span>🤖</span>AI Buddy</button></nav><button id="logout" class="logout">↪ Logout</button></aside><section class="main"><header><div class="header-left"><button id="menu" class="icon mobile">☰</button><div><div class="header-title" id="topTitle">Dashboard</div><div class="header-date" id="topDate"></div></div></div><div class="header-actions"><button id="notifyBtn" class="icon">🔔</button><button id="profileMini" class="mini-avatar"></button></div></header><main id="content"></main></section></div>`;document.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>go(b.dataset.view));$('#aiBtn').onclick=()=>window.open('https://chatgpt.com/','_blank','noopener');$('#logout').onclick=()=>{localStorage.removeItem('sc_token');authPage('login')};$('#notifyBtn').onclick=()=>go('notifications');$('#profileMini').onclick=()=>go('profile');$('#menu').onclick=()=>$('#sidebar').classList.toggle('open')}
function go(v){state.view=v;renderView();$('#sidebar')?.classList.remove('open')}
function renderView(){const titles={dashboard:'Dashboard',profile:'My Profile',timetable:'Timetable',attendance:'Attendance Tracker',subjects:'Subjects & Chapters',tasks:'Study Plan',tests:'Tests',focuslearn:'FocusLearn',notifications:'Notifications'};$('#topTitle').textContent=titles[state.view];$('#topDate').textContent=new Date().toLocaleDateString('en-IN',{weekday:'long',day:'numeric',month:'long',year:'numeric'});const p=state.user.profile||{};$('#profileMini').innerHTML=p.photo?`<img src="${p.photo}">`:esc((p.fullName||state.user.email||'S').charAt(0).toUpperCase());document.querySelectorAll('[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===state.view));const fn={dashboard:dashboard,profile:profile,timetable:timetable,attendance:attendance,subjects:subjects,tasks:tasks,tests:tests,focuslearn:focuslearn,notifications:notifications}[state.view];$('#content').innerHTML=fn();bindView()}
